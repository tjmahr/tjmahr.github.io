...
Each participant has their personal probability of correctly choosing word or nonword. This probability is the **conditional mean** or **subject-specific mean**. The word *conditional* is there to remind that there is a conditional probability or expectation at play.

$$
\textrm{P}(\text{correct}~ |~ \text{participant}_i) \\
\textrm{E}(\text{correct}~|~ \text{participant}_i, n~\text{trials})
$$

But if we remove that conditional part:

$$
\textrm{P}(\text{correct}) \\
\textrm{E}(\text{correct}~|~ n~\text{trials})
$$

we have a **marginal mean** or **population mean** or **population-averaged mean**.

We know from Bayes' theorem that that 
