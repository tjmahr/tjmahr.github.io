...
counterinuitive bullshit. I think analysts will fetishize surprising or
...
report with this image from around May 12, 2020, but nothing came up.
After dredging through a bunch of Morgan Stanley report PDFs, I noticed
that the reports usually had a small number of authors, so I am
wondering whether (and hoping that) the original report was something
more akin to a dashed-off newsletter than a research report.

Failing to find the original image, I tried to recreate it. The original
image credits The COVID Tracking Project, and [their downloads
...
to the time range of the cursed image, and plot the total number of current
ICU patients.
...
There is no jump in ICU patients, and because the jump disappeared when we
used a more recent (and presumably better) version of the dataset, the
jump was probably some kind of artifact. 

Out of curiosity, let's look at the state-by-state data. Because
(spoiler alert) half the states only have `NA` values for this time
period, we will filter out the `NA` points and look at the remaining
points.
...
There it is: the jump ICU patients on May 7th. Let's look at the
state-by-state data:
...
Look at New York (NY). That's the jump in original plot. 

By adding incomplete data from NY to the rest of the states, the 
analyst effectively treated all of the missing points in the NY panel 
as zeros. 
...
*Last knitted on 2022-03-23. [Source code on
