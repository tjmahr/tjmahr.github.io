...
spectrogram_out <- "_R/data/mfa/library-tidyverse-library-brms.Spectrogram"
...
data_spectrogram <- "_R/data/mfa/library-tidyverse-library-brms.wav" |> 
...
data_spectrogram |> 
  ...
  readr::write_csv("_R/data/mfa/library-tidyverse-library-brms.csv")
```

In this post, I announce the release of
[readtextgrid 0.2.0][rtg-cran] R package, describe the problem that
the package solves, and share some thoughts on AI-assisted programming.

## Textgrids are way to annotate audio data
...
scripting language for manipulating and analyzing data and for automating
annotation workflows. Users can annotating intervals or points of time 
in a sound file using **textgrid** object. Here is a screenshot of a
textgrid in Praat:
...
1.  Amplitude wave form, showing intensity over time
2.  Spectrogram, showing how the intensity (*color*) at frequencies (*y*) changes over
    time (*x*). Red dots mark estimated formants (resonances) in the speech signal. 
...
acoustic model to lay down the time intervals of each word and each
...
This package does not have `write_textgrid()` function. I prefer to use 
Praat's own scripting language for modifying textgrids.
...
[Josef Fruehwald][jf-1], a linguist with [lots of acoustics/phonetics software][jf-2], submitted a pull request to do a proper parser that I eventually rewrote to handle various edge cases and undocumented behavior in the .TextGrid specification. I made an [adversarial .TextGrid file][hard-file] 😈 that could still be opened by `praat.exe` but was meant to be difficult to parse.
...
The new R character-by-character-based parser worked 🎉. It had conquered
...
could say about them. As a language scientist, I'll start here: They
...
[bullshit]: https://www.andrewheiss.com/ai/#text "Andrew Heiss's AI positiion statement"
...
[^fn-reasoning]: There is something introspective about 
    ...
    work through them. But still, I'm thinking about what the ground truth in 
...
[boilerplate or templates][usethis] to fill out. For example, I ask
an LLM to "write some unit tests for a function `round_to(xs, unit)`
that rounds a vector of values to an arbitrary unit" and receive:
...
[^fn-tradition]: I think there is a great "tradition"---not sure of the right 
    ...
    but at a literal level: Code that sets a foundation for self-directed
    learning.
...
To be clear---I can't believe I need to make this disclaimer---we should
always care about truth and accuracy when we write prose and publish it
and put our name on it. Vibes are not scientific or scholarly. When I
see emails or code documentation with immaculate formatting and perfect
language, my bullshit sensor goes off and I worry that I need to read
extra carefully because a smooth-talking robot is trying to pull a fast
one on me. I don't use LLMs for writing except for proofreading or
requests for nitpicking. I have an instruction in ChatGPT that says not
to revise anything I write unless it sneaks in Magic: The Gathering card
names into the output. (It generally ignores that rule of mine,
giving me joyless unsolicited edits.)

In this mini-position statement on LMM assistance, the two principles I am 
trying to develop are:
...
So we need to be mindful of this limitation and skeptical of the output.
We should audit the results and make sure we comprehend them, or admit
upfront that this code is running on vibes. 

Sam Mehr has a stance I also quite like. LLMs are fine for code we don't
feel bothered to learn:
...
learning and what problems are worth solving the hard way is very 
useful for an LLM user. 
...
faster, until it failed a unit test on a character wearing diacritic on
it. After a few rounds of trying to improve the loop, I asked quite
...
[usethis::use\_cpp11()](https://usethis.r-lib.org/reference/use_cpp11.html)
to make the necessary boilerplate, and I had to troubleshoot the first
couple versions of the function because of errors. The [cpp11
...
asked the LMM a lot of follow-up questions: what does `auto` do, what is `size_t`
doing, and so on. And I annotated the C++ code with comments for my
own understanding.
...
how the character `é` for example has the codepoint (number) `U+00E9` in
Unicode, so it falls in the range of codepoints that need to be split
into two bytes. The [scheme for two-byte encoding][unicode-table] is 
...
This new C++ based tokenizer yielded a *massive* performance gain:
...
One downside of converting adopting cpp11 is that the package needs to compile 
code. As a result, I can't just tell people to try the developer version of 
the package with `remotes::install_github()`. CRAN compiles packages for users
so end users don't face this issue when installing the official released 
version of packages on CRAN.

One workaround I adopted here relying on [R
...
<!-- There were a couple of annoying moments along the way. I needed a way to convert -->
<!-- the character tokens representing numbers into actual numeric values in -->
<!-- R. ChatGPT suggested that I use R's own number conversion low-level -->
<!-- function `R_strtod()`. (I had never considered that by stepping into the -->
<!-- C++ level, R's own low-level functions are available to me and that -->
<!-- seemed very attractive.) Ultimately, that function was too strict  -->
<!-- compared to the behavior of Praat, if I recall correctly,  -->

<!-- But ChatGPT told me that the function "Uses R’s -->
<!-- numeric parser (respects current R locale and NA handling)." Which would -->
<!-- be a problem because the textgrid format only respects the "C" locale for  -->
<!-- numbers. I ended up using  -->
...
wood from a 6-foot piece of lumber.
