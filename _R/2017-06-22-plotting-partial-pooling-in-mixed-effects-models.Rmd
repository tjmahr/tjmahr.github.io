...
a_sleepstudy <- downlit::autolink_url("lme4::sleepstudy")
a_statsmooth <- downlit::autolink_url("ggplot2::stat_smooth")
a_facetwrap <- downlit::autolink_url("ggplot2::facet_wrap")
a_lmlist <- downlit::autolink_url("lme4::lmList")
a_geomabline <- downlit::autolink_url("ggplot2::geom_abline")
a_coefmermod <- downlit::autolink_url("lme4::coef.merMod")
a_ellipse <- downlit::autolink_url("ellipse::ellipse")
a_coordcartesian <- downlit::autolink_url("ggplot2::coord_cartesian")
a_armdisplay <- downlit::autolink_url("arm::display")
a_statdensity2d <- downlit::autolink_url("ggplot2::stat_density_2d")
```
...
[`sleepstudy`](`r a_sleepstudy`) dataset from the lme4 package. The
...
[`facet_wrap()`](`r a_facetwrap`) to create subplots for each
participant and [`stat_smooth()`](`r a_statsmooth`) to create a
...
other participants exist. The [`lmList()`](`r a_lmlist`) function in
...
[`geom_abline()`](`r a_geomabline`) to draw the lines from our dataframe
...
and slope using [`coef()`](`r a_coefmermod`).
...
The [`ellipse()`](`r a_ellipse`) function takes a covariance matrix, a
...
[`coord_cartesian()`](`r a_coordcartesian`). (Your aesthetic preferences
...
We get a similar overview as [`arm::display()`](`r a_armdisplay`) when
we print the model.
...
[`stat_density_2d()`](`r a_statdensity2d`). We set the coordinate limits
