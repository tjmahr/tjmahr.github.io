...
The mean in our simulations `r round(mean(simulation), 1)` is very close to
the analytic solution of 100.2. The median `r median(simulation)` is less than
...
`r max(simulation)` encounters.

We can visualize the distribution with a histogram.

```{r plot-encounters, fig.cap = "A long-tail of unfortunate RNG for Unown completionists"}
library(ggplot2)
...
encounters over the course of `r round(100 / 12, 1)` years. As I said, a cruel
...
```{r plot-unown-home-stretch, fig.cap = "The painful home stretch of Unown completion"}
ggplot(df_results) + 
