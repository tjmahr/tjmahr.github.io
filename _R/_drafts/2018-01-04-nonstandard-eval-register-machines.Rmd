...
I recently completed all 25 days of Advent of Code 2017, an annual series of
recreational programming puzzles. Each day describes a programming puzzle and
illustrates a few simple examples of the problem. The whole thing has a
backstory involving Santa Claus and his elves; it's charming. Then they pull the
rug out from under you, as you have to solve a really large version of that
problem.

For five or so of the days, I used special or nonstandard evaluation to
...
Day 8 requires us to simulate the state of a register machine as it receives a
...
Each user, I think, receives some unique input for the problem. My problem input
contains 1,000 of these instructions.

```{r}
library(magrittr)
full_input <- file.path("https://raw.githubusercontent.com", "tjmahr", 
                        "adventofcode17", "master", "inst", 
                        "input08.txt") %>% 
  readr::read_lines()
...
the problem basically becomes as hard as running an R script.


## Parsing the register instructions with regular expressions

The instructions have a very simple grammar. Here is how I would label the first
...
will walk through each of the regular expression patterns needed to parse the
instructions.

To match the verbs, we can just use the [or `|` operator](https://www.regular-expressions.info/alternation.html), so
...
Numbers here are just integers, and sometimes they are negative. A number here
...
with a column for each matched group. I am going to include an extra set of
...
Now we can package this step into a function that that takes an instruction's
