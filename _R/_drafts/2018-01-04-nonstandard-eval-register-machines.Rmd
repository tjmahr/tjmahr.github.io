...
I recently completed all 25 days of 
[Advent of Code 2017](http://adventofcode.com/2017), an annual series of
...
of a register machine as it receives a
...
We can package this step into a function that that takes an instruction's
...
something I've described as a kind of
[bottled up magic spell](/set-na-where-nonstandard-evaluation-use-case/): An 
...
template with the parts of the parsed line. Because `inc/dec` are just addition
and subtraction, we replace them with the appropriate math operations.
...
We have to figure out _where_ we want to evaluate the generate R code. We create
a register object to hold the values. The object will just be a `list()` with
some extra metadata. This object will also serve as the location where the
machine instructions are evaluated.
...
The function `eval_tidy()` lets us evaluate an R expression inside of R data 
object.
...
# For quick testing, we can also pass in quoted expressions
create_register_machine(a = 2, b = 0) %>% 
...
the register names, initialize them to 0 and then evaluate the 
...
evaluation**. R throws an error if you ask it to evaluate a variable that
doesn't exist. But here we are going to detect those missing variables and set
them to 0 before they get evaluated.
...
Let's process all 1,000 instructions. We parse and evaluate them one at a
time.
...
I like this kind of nonstandard evaluation solution of converting problems into
...
machine simulation, the nonstandard evaluation is pretty obvious. But it's 
also a viable approach when the instructions are more subtle, as in 
[this problem about simulating "dance" moves](http://adventofcode.com/2017/day/16).


[^starthere]: That actually would be pretty easy. Get a dataframe with
`purrr::map_df(full_input, parse_instruction)`. Find the unique register names.
Create a list of 0's with those names. Use `do.call()` to call
`create_register_machine()` with that list.
