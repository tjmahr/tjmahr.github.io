...
like `@SimpsonFlanders2006 found...` will render as "Simpson & Flanders (2006)
...
stuff. But I have colleagues who will read a draft of a manuscript and write
unnecessary comments about how to cite stuff in APA. And the problem is just
subtle and pervasive enough that it doesn't make sense to fix manually. My
current project has 15 of these ill-formatted citations. That number is just
big enough to make manual corrections too much of a hassle and error-prone---
easy to miss 1 in 15.

## Find and replace

Here is a quick R function I wrote that replaces all those inline ampersands
with "and"s.
...
Let's assume that an inline citation consists of an author's last name followed
...
In the hotdog example, positive lookahead `hot(?=dog)` says find "hot" if it's
follows by "dog".

We use positive lookahead to find only the ampersands followed by an author
year. We replace the strings that match this pattern with and's.
...
`` `r re_ampersand_author_year` ``. It's unreadable as it is, but that's why
we built it up step by step from easier sub-patterns like `re_author` and
`re_inline_year`.

## Steps towards production

These are complications that arose as I tried to use the function on an actual
manuscript:
...
I have to do a bit of fussying around by finding the line `"## References"` and
