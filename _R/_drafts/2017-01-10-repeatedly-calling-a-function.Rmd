...
book-keeping. Generally, if we are iterating over the structure of something, like
...
are that solved by `Map()` or `lapply()`. That's why we hade to invent our own
...
there are zero repetitions, the work is done and we return the result. In the
...
Another drawback to the recursive approach is its performance. It's a little
