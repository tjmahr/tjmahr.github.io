...
example critters so that we can get some intuition about the data in this
scaling. Also, I wanted to try out the [annotation tips][r4ds-labels] from the 
R4DS book.
...
Now, we plot the regression results to show the predicted mean of _y_ and its
95% confidence interval. This task is readily accomplished in ggplot2 using
`stat_smooth()`. This function fits a model and draws the mean and CI for each
...
interval has [a frequentist interpretation][interval-interp](https://stats.stackexchange.com/questions/2272/whats-the-difference-between-a-confidence-interval-and-a-credible-interval) which can be unintuitive for this sort of data. 
...
the values of _x_. When these line pile up on top of each other, they create
an uncertainty band around our line of best fit. More plausible lines are more
likely to be sampled, so more plausible lines overlap and create a uniform color
around the median line. As we get farther away from the mean of _x_, the lines
start to fan out and we see very faint individual lines.
...
of new data. We say _means_ because it generates the predicts for each our
samples from the posterior. The result is 4000 x 80 matrix of fitted means.
...
mean), and then do a table-join with the `observation` column I included in
`new_data`.

```{r}
df_pred_lin <- pred_lin %>% 
  as_data_frame %>% 
  setNames(seq_len(ncol(.))) %>% 
  tibble::rownames_to_column("posterior_sample") %>% 
  tidyr::gather(observation, fitted, -posterior_sample)
df_pred_lin
...
df_pred_lin <- df_pred_lin %>% 
  ...
  summarise(median = median(fitted),
            lower = quantile(fitted, .025), 
            upper = quantile(fitted, .975)) %>% 
  left_join(new_data, by = "observation")
df_pred_lin
```

Now, we can do the line-plus-interval plot.
...
Although the interpretation of the interval changes, its location barely changes
at all. If we add the `stat_smooth()` layer onto our last plot, we can see that
two sets of intervals are virtually identical. With this much data, both types
of models can make very similar estimates.
...
(As a child growing up on a dairy farm, it stuck out to me how I never saw cows sleeping. Were they okay? Are they constantly tired and groggy? They just don't need much sleep.)

[ggplot2-mammals]: https://vincentarelbundock.github.io/Rdatasets/doc/ggplot2/msleep.html
[r4ds-labs]: r4ds.had.co.nz/graphics-for-communication.html#annotations
