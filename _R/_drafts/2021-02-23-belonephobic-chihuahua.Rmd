...
share: true
header:
  overlay_image: "assets/images/2021-02-spline.png"
  image_description: "A drafting spline"
  overlay_filter: rgba(10, 10, 10, 0.1)
  caption: "Drawing credit: [**Pearson Scott Foresman**](https://commons.wikimedia.org/wiki/File:Spline_(PSF).png)"
---
...
What this means is that we can use nlme or lme4 to estimate a smooth as
mixed effects model. mgcv provides this feature in its aptly named `smooth2random()` function.

- link

Here you can see the extent of the internal transformation needed to
convert our nice wiggly cubic regression basis into a mixed effects
framework.
...
I probably won't be converting smoothing bases into mixed effects model
matrices and fitting them with lme4 anytime soon, but it's useful to
know about this fact because of the following fact.
...
`stimes_1`? That the single fixed effects term from above. I'm pretty sure about this because I learned `smooth2random()` from studying the brms source code.

Now, if we look at the actual Stan code used to fit the model, we see a
mixed effects model. **You can skim this. The important parts are the
...
mixed model is call the "non-centered parameterization". It's one of
...
You see completely analogous code in the Bayesian radon model above.
...
penalty matrix. Indeed, mgcv provides a random effects (`"re"`) smoother
basis so we can estimate our mixed model from above using a smooth.
...
random effect variance estimate for us with gam.vcomp() (variance
components)

```{r}
gam.vcomp(gam_radon) %>% round(3)
```
...
smooths is that what the effective degrees of freedom tells us:
...
we ask brms to use its LOO method to estimate the effective number of
...
both seem to telling us something similar about how many much
information in `county` variable we have after partially pooling down model
parameters.
...
The purpose of this post was to demystifying the connection between
penalized smooths and mixed effects models. Formally, it all boils down to a
hyperparameter that penalizes model coefficients (or pools information) so that the
model uses a smaller number of effective parameters. This connections
enriches the connection between the two: A by-county random intercept is
a category smoother. We can estimate a smooth by putting a prior on the
variance of the basis function weights (provided that we incorporate the 
penalty matrix somehow).

Random effects are funny things. We are first taught them in a repeated
measures framework as a way to fix to handle a problem in the data
...
this problem between repeated-measures random effects and smoothing
random effects in their paper Random Effects Old and New, and I think
it's worth remembering this distinction when we use the phrase "random
effects".
