...
how a logistic growth curve works. I made this figure for a conference
submission. I had a tight word count limit (600 words) and a complicated
...
For the statistical models, we expected children 
...
plot_priors <- function(x, n, asymptote = rbeta(n, 10, 20),
  ...
  data <- samples %>%
    purrr::pmap_dfr(
      function(sample_num, asymptote, midpoint, scale) {
        tibble::tibble(
          xs = xs,
...
plot_priors(0:96, n = 100, asymptote = rconstant(n, .8), scale = rnorm(n, .1, .04))
...
My title says assistatn scientist, but I say data scientist because I still do
all my work in RStudio. And I study development
...
f <- bf(
  weight ~ asym * inv(1 + exp((mid - day) * scale)),
  asym ~ 1 +  (1 | ID | plot),
  mid ~ 1 +  (1 | ID | plot),
  scale ~ 1 + (1 | ID | plot),
  nl = TRUE)
...
f2 <- bf(
  weight ~ asym * inv(1 + exp((mid - day) * scale)),
  asym ~ 1 + phenotype + year + (1 | ID | plot),
  mid ~ 1 + phenotype + year + (1 | ID | plot),
  scale ~ 1 + phenotype + year + (1 | ID | plot),
  nl = TRUE)
...
plot_priors(d$day, n = 100, asymptote = rnorm(n, 20, 10), midpoint = rnorm(n, 40, 10), scale = rnorm(n, .5, .25))

prior_fixef <- c(
  prior(normal(20, 10), nlpar = "asym", coef = "Intercept"),
  prior(normal(40, 10), nlpar = "mid", coef = "Intercept"),
  prior(normal(.5, .25), nlpar = "scale", coef = "Intercept"),
  prior(normal(0, 10), nlpar = "asym", class = "b"),
  prior(normal(0, 7), nlpar = "mid", class = "b"),
  prior(normal(0, .25), nlpar = "scale", class = "b"),
  prior(exponential(1), class = "sigma")
  # prior(exponential(1), dpar = "sigma")
)
...
#   prior = c(prior_fixef, prior_ranef))
# 
# l <- make_standata(f3, data = d,
#   prior = c(prior_fixef, prior_ranef))
# 
# m_comp <- rstan::stan_model(model_code = m)
# m_fit <- sampling(m_comp, l)
# 
# 
...
# intels <- d %>%
#   distinct(day, trt, year, phenotype, plot)
...
d_fit <- d %>% 
  bind_cols(fit %>% resid() %>% as_tibble()) %>% 
  bind_cols(fit %>% fitted() %>% as_tibble() %>% select(Fitted = Estimate)) 
  
d_fit %>% 
  filter(year == "y1990", phenotype == "PI") %>% 
  ggplot() + 
    aes(x = Fitted, y = Estimate) + 
    geom_pointrange(
      aes(ymin = Q2.5, ymax = Q97.5), alpha = .4) + 
    geom_line(
      aes(group = plot)) +
  scale_x_log10() +
  labs(y = "Residual")
